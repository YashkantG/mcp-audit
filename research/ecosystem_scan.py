...
def aggregate(results: list[dict]) -> dict:
    ...
    return {
        ...
        "pct_repos_with_finding": round(100 * repos_with_finding / scanned, 1) if scanned else 0.0,
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp_eco_") as tmp:
        ...
        for i, slug in enumerate(sample, start=1):
            ...
            if res is None:
                failed += 1
                print(f"[{i}/{len(sample)}] {slug}: skipped (clone/scan failed)")
                continue
            results.append(res)
            n = len(res["findings"])
            print(f"[{i}/{len(sample)}] {slug}: {n} finding(s)")

    summary = aggregate(results)
    summary["repos_skipped"] = failed
